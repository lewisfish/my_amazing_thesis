...
S. Smirni \textit{et al.} took autofluorescence spectra from the volar forearms of volunteers using the LAKK-M multifunctional laser non-invasive diagnostic system (MLNDS).
...
The LAKK-M system delivers and collects light via the same optical fibre package.
The fibre package has 7 optical fibre, one for each laser wavelength (365~nm, 430~nm, 532~nm, 635~nm), two detectors, and a spectrometer.
The fibres radii are 0.4~mm, and the separation between the fibres is around 1~mm.
...
As before the simulation was setup to match the above experimental procedure.

\Cref{fig:NMupdate} show the output of the \gls*{nm} method during a run.
