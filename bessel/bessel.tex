...
From using Airy beams to move particles and cells~\cite{baumgartl2008optically}, Bessel beam ``tractor beams''~\cite{ruffner2012optical}, Airy and Bessel beams for better field of view in light-sheet microscopy~\cite{vettenburg2014light}, and utilising Laguerre-Gaussian beams to trap optically reflective particles~\cite{simpson1996optical}.
...
For example, traditional \gls*{mcrt} methods cannot model Gaussian beams, as Gaussian beams have a finite beam waist at their focus.
...
\begin{equation}
U(P)=\frac{1}{i\lambda}\iint\limits_{\Sigma} \frac{Ae^{ik(\rho+r)}}{\rho r} e^{-i\omega t}cos(\theta)\ dS
\label{eqn:disturb}
\end{equation}
...
\begin{equation}
\rho + r \approx \rho_0 + r_0 + (y^2+z^2)\frac{\rho_0 + r_0}{2\rho_0r_0}
\label{eqn:binomial}
\end{equation}
...
\begin{equation}
U(P)=\frac{Ae^{-i[k(\rho_0+r_0)\omega t]}}{i\lambda\rho_0r_0}\iint\limits_{\Sigma} e^{i\pi y^2\tfrac{(\rho_0+r_0)}{\lambda\rho_0r_0}+i\pi z^2\tfrac{(\rho_0+r_0)}{\lambda\rho_0r_0}} \ dS
\label{eqn:midway}
\end{equation}
...
\begin{equation}
\int_{0}^{w}e^{\frac{i\pi w'^2}{2}}dw'=C(w)+iS(w)
...
\begin{equation}
I_p = \frac{I_u}{4} \{[C(u_2) - C(u_1)]^2 + [S(u_2) - S(u_1)]^2\} \times \{[C(v_2) - C(v_1)]^2 + [S(v_2) - S(v_1)]^2\}
...
However, shrinking the medium's size in the \textit{x} and \textit{y} directions gives the same effect as using a smaller input beam.
...
T_3(r,\varphi)&=T_1*T_2=e^{-ik_rr}e^{-il\phi}
\end{align}
