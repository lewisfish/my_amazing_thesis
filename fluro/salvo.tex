...
Dermatologists categorise the skin into several layers based upon the morphology, function, and contents of each layer~\cite{freedberg1999fitzpatrick,zaidi2010dermatology}.
...
Therefore, we simplify this into just five layers: stratum corneum, epidermis, papillary dermis, reticular dermis, and hypodermis, with the epidermis compromising of stratum lucidum, stratum granulosum, stratum spinosum, stratum basale.~\Cref{fig:skinexample} shows the geometry of this model.
...
\subsubsection*{Stratum Corneum} % (fold)
\label{sub:stratum}
...
% subsection stratum_corneum (end)
...
\begin{equation}
\mu_{a,Oxy/DeOxy}=150\ \ln10\ \frac{\varepsilon_{Oxy/DeOxy}}{64458}\quad cm^{-1}
\label{eqn:oxy}
\end{equation}
...
\Cref{fig:absorcoeff} shows the calculated absorption spectra for the above chromophores as a function of wavelength.
...
\Cref{tab:optpropsvals} shows the amount of each chromophore that is included in each layer and the size adopted for each layer as well as the refractive index for each individual layer.
...
The first term represents the Rayleigh scattering terms ($f_{ray}$), whilst the second represents the Mie scattering term ($1-f_{ray}$).
Essentially the Rayleigh term allows the treatment of scattering by particles smaller than the incident wavelength, and the Mie terms accounts for scattering by particles larger than the incident wavelength.
...
As information on wavelength dependent refractive indices is not readily available, we assume that the refractive indices are the same for all wavelengths used in this chapter. This means that the fluence near the boundaries of the skin layers will either be over or underestimated depending on how close these assumed values are to reality.

\subsection{Fluorophores in the Skin}

As discussed in~\cref{sec:salvointro} above, there are various molecules and proteins responsible for the autofluorescent response of the skin.
...
% stratum        nadh, fad, tyrosine, tryptophan
% epidermis    nadh, fad, tyrosine, tryptophan
% pap            nadh, elastin, collagen, tyrosine, tryptophan
% ret            nadh, elastin, collagen, tyrosine, tryptophan
% hypo           nadh, elastin, collagen, tyrosine, tryptophan
...
The effect of tissue optics on a given signal is not easily elucidated, and thus \gls*{mcrt} must be used to compute this.
This leaves the problem as an optimisation problem of finding the optimal fluorophore confrontations, locations and types.
...
This makes it ideal for problems where derivatives can not be computed easily, or the search space is not smooth.
However, the NM method can also get stuck at local minima so care must be taken to avoid this.
This is done by running the Nelder-Mead multiple times with different starting points and testing to see if they arrive at the same answer.
...
\Cref{eqn:svgfiltereqn} shows the functional form of the Savitzky-Golay filter, where for a data set consisting of $n$ $(x_j,y_j)$ points, and $C_i$ are the set of $m$ convolution coefficients.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{sgfilter-prrof.pdf}
...
Using the above set-up with the Savitzky-Golay filter on the output spectrum, allowed the \gls*{nm} method to efficiently run many models of various different concentrations and ``find'' a set of concentrations that resulted in a close match with the target spectrum.
...
\Cref{fig:spaceplot2D} shows the search space and the spread of the concentration values calculated by the \gls*{nm} method compared to the original target concentrations.
...
Finally, we analyse the effect excitation wavelength has on the detected signal of NADH, FAD, elastin and collagen.
...
\Cref{fig:nadheem} shows the \gls*{eem}s for NADH and FAD, and~\cref{fig:elastineem} shows the \gls*{eem}s for elastin and collagen.
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{NADH-eem.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{FAD-eem.pdf}}}%
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{elastin-eem.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{collagen-eem.pdf}}}%
...
\Cref{fig:nadhfad-blood,fig:elscol-blood} show the results from these models.

\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{nadh-blood.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{fad-blood.pdf}}}%
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{elastin-blood.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{collagen-blood.pdf}}}%
...
For elastin and collagen, the autofluorescent spectra are not as simple as that of NADH or FAD\@.
...
\Cref{fig:nadhfad-mel,fig:elscol-mel} show the results of this investigation.

\begin{figure}[!htbp]
    \squeezeup{6.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{nadh-melanin.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{fad-melanin.pdf}}}%
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{elastin-melanin.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{collagen-melanin.pdf}}}%
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{nadh-thickness.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{fad-thickness.pdf}}}%
...
\Cref{fig:nadhfad-thick,fig:elscol-thick} show some of the results of the effect of skin thickness on the autofluorescent signal from several fluorophores used in this study.
...
\begin{figure}[!htbp]
    \squeezeup{3.5}
    \centering
    \subfloat{{\includegraphics[width=.46\textwidth]{collagen-thickness.pdf}}}%
    \subfloat{{\includegraphics[width=.46\textwidth]{elastin-thickness.pdf}}}%
...
This ``cleaning'' process was required as the whole spectrum was shifted by $20~nm$ into longer wavelengths with respect to the included backscatter peak.
The cause of this systematic shift is unknown, but is most likely due to mis calibrated or faulty equipment.
...
The figure shows that as the simplex iterates, the average fitness improves, and the average number of calls to the \gls*{mcrt} algorithm tends to $\sim 1.4$.
...
There are several probable reasons why the spectrum generated by the \gls*{mcrt} and \gls*{nm} does not match the experimental data exactly (see~\cref{fig:bestNMresult}).
