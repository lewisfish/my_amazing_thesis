...
Despite their appeal using autofluorescence to diagnose and assess disease risk more research needs to be carried out on various unknowns: information on the location of fluorescence, which fluorophores contribute to the signal and how much, how much the optics of tissue affect the signal, and the variability of the signal over different location on the body.
...
Furthermore, information on the fluorophores used in this works quantum yield's is not readily available.
...
The signal is the recorded by an \gls*{obs}.
...
The fiber package has seven optical fibers, one for each laser wavelength (365~nm, 430~nm, 532~nm, 635~nm), two detectors, and a spectrometer.
...
\begin{figure}[!htpb]
  ...
  \caption{Example of a PORH test whilst measuring autofluorescence and perfusion. The perfusion of blood in the skin decrease once the cuff is inflated, and then rapidly reaches a maximum once the cuff is removed. The inverse of this can be seen in the autofluroescent response of NADH in the tissue.}
  \label{fig:porh}
\end{figure}

S. Smirni collected around x spectra at the rate of 1 per minute over a period of blah minutes as the \gls*{porh} test was undertaken.
5 for a baseline before inflation of the cuff, blah during inflation and blah after inflation.


\begin{figure}[!htpb]
  \centering
  % \includegraphics[width=0.5\textwidth]{}
  \caption{Figure shows an example of a raw spectrum taken by S. Smirni \textit{et al.}.}
...
The figure shows the fluence of light for each fluorophore,***add in elastin*** that is detected.
...
\begin{figure}[!htpb]
    ...
    \caption{Detected fluence for FAD and NADH fluorescence. Inset show zoom in of top layers of the skin.}
...
For NADH we use wavelength in the range of 250 to 40~0nm, and for FAD we use 250 to 500~nm.
...
***do literature search to see where people measure these flurophores***
...
We model the experimental setup as described in a previous section.
The computational speed ups described in the previous sections are also used.
The 5 layers skin model is setup with NADH, FAD and elastin distributed in different layers.
NADH and FAD is initially in all layers bar the hypodermis, and elastin is contained in just the papillary and reticular dermis.
Each layers has its own concentration of the fluorophores giving $n$ for the \gls*{nm} method of 10.
...
\Cref{fig:NMupdate} show the output of the \gls*{nm} method during a run with the above parameters.
The figure shows that as the the simplex iterates, the average fitness improves, the average number of call to the \gls*{mcrt} algorithm tends to $\sim 1.4$.
...
\begin{figure}[!htpb]
  ...
  \caption{Figure shows the best result from the \gls*{nm} operating on the experimental data from s. Salvo \textit{et al.}. The spectrum has a fitness of $\sim 3$.}
...
There are various source that have measured them, but they are usually confined to several wavelengths of the authors interest and not a full spectrum.
...
After a literature search, the peaks for NADH, FAD and elastin seem to be in the range blah, blaahh, and gahd + sources.
Whereas the creators of the LAKK-M put the peaks at gsd, dsd, and fds.
