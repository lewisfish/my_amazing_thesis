...
The first concept we add to the MCRT method is assigning a phase to each packet.
...
The double slit experiment, is a simple experiment where monochromatic plane wave of light is incidence on two slits distance apart $d$, and width $b$, and the interference pattern is observed on a screen a distance $L$ away from the slits.
...
The simulation was carried out for a wavelength of $488~nm$, a slit width of $10\lambda$, a slit separation of $80\lambda$, and the detector screen positioned $10000\lambda$ away from the slits.
...
The simulation was run with $10^9$ packets, which took $\approx\ 10~mins$ to run on an 8 core Intel Xeon machine.
...
\begin{figure}[!ht]
    ...
    \caption{Comparison of theory and simulation for the double slit experiment.}
...
% implementation of lens's -> aspheric
...
To ensure that the method described in~\nameref{sec:bestheory} works as intended for Bessel beams several tests are run against theoretical expressions and experimental data.
...
Volumes of m, n, l, and p $\mu L$ are used of Intralipid in 500 $\mu L$ of water.
...
To achieve this the Bessel beam was given $1000mW$.%***fill this in
