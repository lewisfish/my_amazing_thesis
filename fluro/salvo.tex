...
\begin{figure}[!htpb]
  ...
  \caption{Figure shows an example of a raw spectrum taken by S. Smirni \textit{et al.}. Figure illustrates how the spectrum is red-shifted, along with some of the artifacts in the data.}
...
NADH and FAD is initially in all layers bar the hypodermis, and collagen is contained in just the papillary and reticular dermis\footnote{we omit any fluorophores form the hypodermis as they are not expected to contribute to the signal at all, or by very much. This also simplifies the problem}.
...
However, collagen contributes to the main peak and is multi peaks, with peaks at $\sim$380~nm and $\sim$ 465~nm.
...
The reason that the collagen signal it multi-peaked is due to the tissue optics.
There is a large peak at around 400-410~nm in the absorption spectra in the dermis layers.
...
From this chapters work on see how tissue optics affects the fluorescent signal, it cannot account for the discrepancy.
One possible cause for this is that the UV light they use to probe autofluorescence is an LED.
LED light sources are not monochromatic, but rather have a range of wavelength they emit, on the order of 25~nm FWHM.
