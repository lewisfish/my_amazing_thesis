...
We can also investigate the time it takes for different areas of the tissue to become thermally damaged. This can be easily achieved by saving the time each voxel passes one of the damage boundaries in~\cref{eqn:thermalbound}.~\Cref{fig:thres1,fig:thres2} show the minimum time taken for $1^{st}$, $2^{nd}$, and $3^{rd}$ degree burns to occur for both 30~$W$ and 70~$W$ powered lasers.
The 70~$W$ laser shows that there is little to no time (upon the order of *insert*) between $1^{st}$ and $2^{nd}$ degree burns, and around $\sim$~$0.1$~$s$ between $2^{nd}$ and $3^{rd}$ degree burns.
