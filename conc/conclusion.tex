...
The chapter gave examples of how the Monte Carlo method can be used to sample from spectra, and how it can be used to model various physical events.
...
The new method was then used to compare Bessel and Gaussian beams performance in highly turbid media, to see which beam performed ``better''.
\medskip
...
Nanoscale materials, such as gold nanorods, are targeted with a specific wavelength of light (usually near infra-red) which heats up the rods and thus the surrounding tissue, eventually killing the adjacent cells~\cite{singh2016application,gallina2016aptamer}.
...
The algorithm developed as part of chapter 4's work, $\varphi$MC, also has several avenues for future research.
...
Other types of experiments the algorithm could be used for include: laser speckle imaging, focusing light through turbid media, and complex micromanipulation~\cite{vellekoop2007focusing,horstmeyer2015guidestar,vcivzmar2010situ}.
...
A meshed skin model would allow objects like hair, blood vessels, sweat glands, and the uneven boundaries between skin layers greatly increasing the accuracy of the simulations.
...
The power that MCRT has in modelling the transport of light through media, is that the media can be tailored to each individual problem.
...
We used this in the work we presented on our numerical model of tissue ablation.
...
This thesis has also shown that Monte Carlo method is a flexible technique that can be adapted or used in conjunction with other techniques to model complex phenomena, without having to ``gamble'' on other techniques.
