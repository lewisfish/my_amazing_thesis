...
% future work
\section*{Fluorophores in the Skin}
...
% \begin{equation}
% \mu_{a,ret}=((S * \mu_{a,oxy}(\lambda) + (1. - S) * \mu_{a,deoxy}(\lambda) + ln10 * carotene(\lambda) * C_caro + \mu_{(\lambda) * C_bili) *&
%                        B + base(\lambda) * (1. - B)) * (1. - W) + W * Water(\lambda)
