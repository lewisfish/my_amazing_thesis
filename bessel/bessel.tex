...
Where $\varphi~[-]$ is the complex phase of a photon packet, $l\ [m]$ is the distance the photons has travelled, $\lambda~[m]$ is the wavelength of the photon, and \textit{i} is the solution to $x^2=-1$.
Now we can calculate the complex phase of a packet at a position $P_o$, if we know the distance it has travelled, and its original phase,~\cref{fig:phase-diag}.
...
We do not model the interference at a point in space where photons packets cross, due to the ballistic nature of the \gls*{mcrt} simulation this does not occur with enough frequency in order to give a good signal to noise ratio. 
TTherfore, interference takes place in a volume, $dV$, or area element, $dA$, instead.
...
\Cref{eqn:kirchhoffeqn} shows the equation for the complex field at a point on a plane as derived by Kirchoff with the inclination factor.
...
\begin{figure}[!hbtp]
    \centering
    \includegraphics[width=0.75\textwidth]{spherabbr.pdf}
...
\Cref{fig:farfield} shows the profile of the Bessel beam in the far field, where the theory predicts it becomes a circular beam.
...
\Cref{fig:compareexpbessel} shows the results from the experiment and simulation. The simulation shows good agreement with experimental data within experimental and simulation uncertainty.
...
To ensure a fairer comparison the Bessel beams central core width is set to that of the Gaussian beam's waist.
