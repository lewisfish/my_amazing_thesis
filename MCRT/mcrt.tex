...
The size of the mentioned scatters affect how light is scattered and into which direction that light is scattered into.
...
For the first test of a~\gls*{mcrt} code, it is compared against a theoretical expression for fluence as a function of depth.
...
This chapter presented the Monte Carlo method from the basics to its use in radiation transport.
Th \gls*{mcrt} algorithm covered in this chapter is used as the basis of the following chapters.
Each chapter will modify the~\gls*{mcrt} for the problem in that chapter, but the main portion, the propagation of photons remains the same.

This chapter also presented the validation of the code against theoretical and experimental results. 
The code compared well with both of these results.
For each modification of the code in the subsequent chapters, the code needs to validated against, to ensure the modifications are correct.
% \section{Improving the Voxel Tissue Model}\label{sec:improve} %*****needed?


%******* output data i.e fluence need to mention this!!!!*******************************************